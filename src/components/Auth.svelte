...
<script>
    ...
    function login() {
        ...
        auth.signInWithEmailAndPassword(email, password).then(() => {isAuthenticated=true;  d("done"); d("auth")}).catch(e => {
    ...
    function register() {
        ...
        // creating the user
        auth.createUserWithEmailAndPassword(email, password).then(() => {isAuthenticated=true; d("done"); d("auth")}).catch(e => {
            err = `(${e.code}) ${e.message}`
        })
    }
    ...
    function logout() {
        if (auth.currentUser) {
            auth.signOut().then(() => {isAuthenticated=false;d("done"); d("logout")}).catch(e => {
                throw new Error(e)
            });
        }
    }
